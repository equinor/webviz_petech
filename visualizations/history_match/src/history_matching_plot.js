...
export default class HistoryMatchingPlot extends Component {
    constructor(config = {}) {
       ...
       this.confidenceIntervalUnsorted = config.confidenceIntervalUnsorted ? config.confidenceIntervalUnsorted : { low: 0, high: 2};
    ...
    renderTitle() {
        this.container.selectAll('text#title')
            .data([this.data.name])
            .enter()
            .append("text")
            .attr("id", "title")
    ...
    renderPlotContainer() {
        this.plotContainer = this.container.append('g')
    ...
    renderParameterLabels() {
        let labelSelection = this.plotContainer.selectAll("text.ylabel")
        ...
        labelSelection
            ...
            .attr("class", "ylabel")
            .attr("transform", (d, i) => {
                return "translate(" + [this.LABEL_MARGIN, this.yScale(i) + this.LABEL_FONT_SIZE] + ")";
            })
    ...
    renderNegativeBars() {
        let negativeBarsSelection = this.plotContainer.selectAll("rect.bar_neg")
            .data(this.data.negative, (d, i) => i);

        negativeBarsSelection.exit()
            .remove();

        negativeBarsSelection.enter()
            .append("rect")
            .attr("transform", (d, i) => {
            ...
            .attr("width", (d, i) => { return this.xScale(this.data.negative[i] + 0.01) + 'px'; })
            .attr("transform", (d, i) => {
                return "translate(0," + this.yScale(i) + ")";
            });
    ...
    renderPositiveBars() {
        let positiveBarsSelection = this.plotContainer.selectAll("rect.bar_pos")
            .data(this.data.positive, (d, i) => i);

        positiveBarsSelection.exit()
            .remove();

        positiveBarsSelection.enter()
            .append("rect")
            .attr("class", "bar_pos")
            ...
            .on('mouseout', (d, i, nodes) => this.hideTooltip(nodes[i]))
            // initial setup when entering
            .attr("width", (d, i) => { return this.xScale(this.data.positive[i]) + 'px'; })
            .attr("transform", (d, i) => {
                return "translate(" + [this.xScale(this.data.negative[i]), this.yScale(i)] + ")";
            });
    ...
    showTooltip(index, node) {
        this.tooltip.select('text.positive')
            .text(this.data.positive[index].toFixed(2));

        this.tooltip.select('text.negative')
            .text(this.data.negative[index].toFixed(2));

        let [x, y] = d3.mouse(this.parentElement.node());

        this.tooltip
            .attr('transform', `translate(${x + 20}, ${y - 30})`)
    ...
    _renderUnsorted() {
        this.plotContainer
            ...
            .attr("d", "M" + this.xScale(this.confidenceIntervalUnsorted.low) + " " + (this.yScale(this.data.negative.length)) + " L" + this.xScale(this.confidenceIntervalUnsorted.low) + " 10 L" + this.xScale(this.confidenceIntervalUnsorted.high) + " 10 L" + this.xScale(this.confidenceIntervalUnsorted.high) + " " + (this.yScale(this.data.negative.length)) + " Z");

        this.plotContainer.append("text")
            .attr('transform', `translate(0, ${this.notePosition})`)
            .attr("font-size", 20)
            .text("*Visualized confidence intervals use the (conservative) assumption of correlated error within each observation group.");
    }

    _renderSorted() {
        this.lineContainer = this.plotContainer.append('g')
            .attr('transform', 'translate(0,0)');

        const numberOfParameters = this.data.negative.length;

        let transformedData = this.confidenceIntervalSorted.high.map((val, i) => {
            return {xVal: this.xScale(val), yVal: this.yScale(i)}
        }).concat(this.confidenceIntervalSorted.low.slice(0).reverse().map((val, i) => {
            return {xVal: this.xScale(val), yVal: this.yScale(numberOfParameters -1 - i)}
        }));
        ...
        transformedData.forEach((d, i) => {
            ...
            if(i < numberOfParameters) {
                if (i < numberOfParameters - 1) {
                    ...
                    line += `V${transformedData[i].yVal + this.yScale.bandwidth()}`;
                }
            } else {
