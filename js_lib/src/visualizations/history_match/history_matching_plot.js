...
export default class HistoryMatchingPlot extends Component {
    ...
    renderParameterLabels() {
        const labelSelection = this.plotContainer.selectAll('text.ylabel')
        ...
        labelSelection
            ...
            .append('text')
            .attr('class', 'ylabel')
            .attr('transform', (d, i) => `translate(${[this.LABEL_MARGIN, this.yScale(i) + this.LABEL_FONT_SIZE]})`)
    ...
    renderNegativeBars() {
        const negativeBarsSelection = this.plotContainer.selectAll('rect.bar_neg')
        ...
        negativeBarsSelection.enter()
            ...
            .attr('width', (d, i) => `${this.xScale(this.data.negative[i] + 0.01)}px`)
            .attr('transform', (d, i) => `translate(0,${this.yScale(i)})`)
    ...
    renderPositiveBars() {
        const positiveBarsSelection = this.plotContainer.selectAll('rect.bar_pos')
        ...
        positiveBarsSelection.enter()
            ...
            .attr('transform', (d, i) => `translate(${[this.xScale(this.data.negative[i]), this.yScale(i)]})`)
    ...
    _renderSorted() {
        ...
        const transformedData = this.confidenceIntervalSorted.high.map((val, i) => ({ xVal: this.xScale(val), yVal: this.yScale(i) })).concat(this.confidenceIntervalSorted.low.slice(0).reverse().map((val, i) => ({ xVal: this.xScale(val), yVal: this.yScale(numberOfParameters - 1 - i) })))
